...
<div class="page-content">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <p class="edu-page-sub">Modify pricing tier, resource limits, and module permissions for this plan.</p>
        </div>
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" id="pkgDesc" class="form-control edu-input" rows="2"
                                    placeholder="Briefly describe the plan..." oninput="updatePreview()">{{ old('description', $subscriptionPackage->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="pkg-section mb-4">
                    ...
                    <div class="pkg-section-body">
                        ...
                        <textarea name="features_list" id="pkgFeatures" class="form-control edu-input" rows="5"
                            ...
                            oninput="updatePreview()">{{ old('features_list', $featuresText) }}</textarea>
                ...
                <div class="pkg-section mb-4">
                    ...
                    <div class="pkg-section-body">
                        <div class="row g-3">
                            @php 
                                $currentPerms = old('permissions', $subscriptionPackage->permissions ?? []); 
                            @endphp
                                    ...
                                    <div class="perm-group">
                                        ...
                                        <div class="perm-group-body d-flex flex-wrap gap-2">
                                            ...
                                            <label class="perm-chip {{ $isDefault ? 'perm-chip-locked' : '' }}"
                                                for="perm_{{ $loop->parent->index }}_{{ Str::slug($slug) }}">
                                                <input class="perm-chip-input" type="checkbox"
                                                    name="permissions[]"
                                                    value="{{ $slug }}"
                                                    id="perm_{{ $loop->parent->index }}_{{ Str::slug($slug) }}"
                        ...
                        <div class="d-flex flex-wrap gap-4">
                            <label class="pkg-toggle-card" for="isPopular">
                                <input type="checkbox" name="is_popular" id="isPopular"
                                    class="pkg-toggle-input" onchange="updatePreview()" {{ old('is_popular', $subscriptionPackage->is_popular) ? 'checked' : '' }}>
                            ...
                            <label class="pkg-toggle-card" for="isActive">
                                <input type="checkbox" name="is_active" id="isActive"
                                    class="pkg-toggle-input" {{ old('is_active', $subscriptionPackage->is_active) ? 'checked' : '' }}>
                                <div class="pkg-toggle-body">
            ...
            <div class="col-lg-4">
                <div class="pkg-preview-sticky">
                    ...
                    <div class="pkg-preview-card" id="previewCard">
                        ...
                        <div class="pkg-preview-name" id="previewName">{{ $subscriptionPackage->name }}</div>
                        <div class="pkg-preview-desc" id="previewDesc">{{ $subscriptionPackage->description ?: 'Your short description will appear here...' }}</div>
                        ...
                        <div class="pkg-preview-price-wrap">
                            <span class="pkg-preview-price" id="previewPrice">
                                ...
                                <span id="previewPriceAmount">৳{{ number_format($subscriptionPackage->price, 2) }}</span>
                            </span>
                            <span class="pkg-preview-period" id="previewPeriod">/{{ $subscriptionPackage->duration }}</span>
                        </div>
        ...
        <div class="pkg-action-bar">
            <div class="pkg-action-inner">
                ...
                <button type="submit" class="pkg-create-btn">
                    <i class="fa-solid fa-arrows-rotate me-2"></i> Update Package
                </button>
            </div>
        </div>
...
function updatePreview() {
    ...
    if (popTag) {
        popTag.style.display = isPop ? 'block' : 'none';
    }
    ...
    if (featContainer) {
        ...
        if (lines.length > 0) {
            featContainer.innerHTML = lines.map(line => `<li class="pkg-preview-feat-item">${escapeHtml(line)}</li>`).join('');
        } else {
            featContainer.innerHTML = '<li class="pkg-preview-feat-item text-muted fst-italic">Add features above to see them here...</li>';
        }
    }
}
