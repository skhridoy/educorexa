            ...
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    <h2 style="font-weight: 800; color: #1e293b; letter-spacing: -0.03em; margin: 0;">
                        <span style="background: linear-gradient(135deg, #4f46e5, #818cf8); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Define</span> New Role
                    </h2>
                    <p style="color: #64748b; margin-top: 4px; font-size: 0.95rem;">Configure institutional access identity and permission matrix.</p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="info-card mb-4">
                            <div class="d-flex align-items-center mb-4">
                                ...
                                <div class="ms-3">
                                    ...
                                    <p class="mb-0 text-white-50 small">Configure access identity</p>
                                </div>
                            </div>

                            <div class="mb-4">
                                <label class="text-white-50 small mb-2 fw-bold text-uppercase">Role Display Name</label>
                                <input type="text" name="name" class="form-control" style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; padding: 12px 18px; border-radius: 12px;" placeholder="e.g. Senior Teacher" value="{{ old('name') }}" required>
                            ...
                            <div class="mb-0">
                                ...
                                <select name="role_type" class="form-select" style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; padding: 12px 18px; border-radius: 12px;" required>
                                    @foreach($role_types as $type)
                                        <option value="{{ $type }}" {{ old('role_type') == $type ? 'selected' : '' }} class="text-dark">{{ ucwords(str_replace('_', ' ', $type)) }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="edu-panel p-4" style="border-radius: 20px;">
                            <div class="d-flex flex-column gap-3">
                                <button type="submit" class="btn-edu btn-edu-primary w-100 py-3" style="border-radius: 14px; font-weight: 700;">
                                    <i data-feather="shield" class="me-2"></i> Deploy Role
                                </button>
                    ...
                    <div class="check-all-panel">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <p class="mb-0 text-muted small">Select module access levels</p>
                            </div>
                        </div>
                                    ...
                                    <div class="perm-card">
                                        ...
                                        <div class="perm-body">
                                            ...
                                            <div class="form-check-edu" onclick="this.querySelector('input').click()">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="perm-checkbox" id="p_{{ $permission->id }}" {{ (is_array(old('permissions')) && in_array($permission->name, old('permissions'))) ? 'checked' : '' }} onclick="event.stopPropagation()">
