    ...
    <div class="container-fluid">
        ...
        <div class="page-header-card mb-4">
            <div class="page-header-content">
                ...
                <p class="page-subtitle">{{ __('Edit subject name, code, type, and details.') }}</p>
            </div>
        </div>
            ...
            <div class="col-lg-4">
                <div class="form-card">
                    <h5 class="mb-4 fw-bold text-primary">
                        <i class="fa-solid fa-sliders me-2"></i> {{ __('Subject Details') }}
                    </h5>
                    <form action="{{ route('subjects.update', ['tenant' => auth()->user()->school->slug, 'subject' => $subject->id]) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="type" name="type">
                                ...
                                <option value="theory" {{ $subject->type == 'theory' ? 'selected' : '' }}>{{ __('Theory') }}</option>
                                <option value="practical" {{ $subject->type == 'practical' ? 'selected' : '' }}>{{ __('Practical') }}</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="description" class="form-label fw-semibold">{{ __('Description') }}</label>
                            <input type="text" class="form-control" id="description" name="description" placeholder="{{ __('Description') }}" value="{{ $subject->description }}">
                        </div>
            ...
            <div class="col-lg-8">
                <div class="data-table-card">
                    <div class="table-header p-3 border-bottom">
                        <h5 class="table-title mb-0 fw-bold"><i class="fa-solid fa-list me-2 text-indigo-600"></i> {{ __('All Subjects') }}</h5>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table data-table mb-0 align-middle">
                            <thead class="bg-light">
                                <tr>
                                    <th class="py-3 px-3">{{ __('Subject') }}</th>
                                    <th class="py-3 px-3 text-center">{{ __('Code') }}</th>
                                    <th class="py-3 px-3 text-center">{{ __('Type') }}</th>
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $subj->id == $subject->id ? 'table-active fw-bold' : '' }}">
                                    ...
                                    <td class="text-center px-3 text-capitalize" style="font-size:0.8rem;">
                                        {{ $subj->type ? __($subj->type) : __('Theory') }}
                                    </td>
