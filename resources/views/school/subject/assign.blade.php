                            ...
                            <div class="mb-3">
                                ...
                                <select id="subject_id" name="subject_id" class="form-select" required>
                                    ...
                                    @foreach($subjects as $subject)
                                        <option value="{{ $subject->id }}" data-type="{{ $subject->type }}">{{ $subject->code ? $subject->code . ' - ' : '' }}{{ $subject->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            {{-- ── Dynamic Mark Fields based on Subject Type ── --}}
                            <div id="mark-fields-container">
                                <div class="alert alert-info py-2 px-3 mb-3" style="font-size: 0.82rem; border-radius: 8px;" id="mark-fields-hint">
                                    <i class="fa-solid fa-info-circle me-1"></i> {{ __('Select a subject to see mark fields') }}
                                </div>

                                {{-- Theory marks (shown for theory & theory_practical) --}}
                                <div id="theory-mark-group" style="display:none;">
                                    ...
                                    <div class="row g-2 mb-3">
                                        <div class="col-6">
                                            ...
                                            <input type="number" id="theory_full_mark" name="theory_full_mark" class="form-control form-control-sm" placeholder="e.g. 75" min="0">
                                        </div>
                                        <div class="col-6">
                                            <label class="form-label fw-semibold" style="font-size: 0.82rem;">{{ __('Pass Mark') }} <span class="text-danger">*</span></label>
                                            <input type="number" id="theory_pass_mark" name="theory_pass_mark" class="form-control form-control-sm" placeholder="e.g. 25" min="0">
                                        </div>
                                    </div>
                                </div>

                                {{-- Practical marks (shown for practical & theory_practical) --}}
                                <div id="practical-mark-group" style="display:none;">
                                    ...
                                    <div class="row g-2 mb-3">
                                        <div class="col-6">
                                            <label class="form-label fw-semibold" style="font-size: 0.82rem;">{{ __('Full Mark') }} <span class="text-danger">*</span></label>
                                            <input type="number" id="practical_full_mark" name="practical_full_mark" class="form-control form-control-sm" placeholder="e.g. 25" min="0">
                                        </div>
                                        <div class="col-6">
                                            <label class="form-label fw-semibold" style="font-size: 0.82rem;">{{ __('Pass Mark') }} <span class="text-danger">*</span></label>
                                            <input type="number" id="practical_pass_mark" name="practical_pass_mark" class="form-control form-control-sm" placeholder="e.g. 8" min="0">
                                        </div>
                                    </div>
                                </div>

                                {{-- Total summary (shown for theory_practical) --}}
                                <div id="total-mark-summary" style="display:none;" class="mb-3">
                                    <div class="p-2 rounded-2" style="background:#f1f5f9; font-size: 0.82rem;">
                                        <span class="fw-bold text-dark">{{ __('Total:') }}</span>
                                        {{ __('Full') }} = <span id="total-full-display" class="text-primary fw-bold">—</span>,
                                        {{ __('Pass') }} = <span id="total-pass-display" class="text-success fw-bold">—</span>
                                    </div>
                                </div>
                            </div>
...
@section('customJs')
    <script>
        ...
        function updateMarkFields() {
            ...
            if (!subjectType) {
                $hint.show(); $theory.hide(); $practical.hide(); $total.hide();
                return;
            }
        ...
        $('#subject_id').on('change', updateMarkFields);

        $(document).on('input', '#theory_full_mark, #theory_pass_mark, #practical_full_mark, #practical_pass_mark', function() {
            updateTotalSummary();
        });
